---
title: "SDSE_transmission_networks_comm_3"
author: "Ouli Xie"
date: "07/06/2023"
output: pdf_document
---
...
## Generate community 3 transmission networks

Note that community 3 is more sparsely sampled compared to community 1. This
results in less inferred transmission links.

```{r, echo=TRUE, results=FALSE, message=FALSE}
sdse_df <- read_csv("sdse_clusters_comm_3.csv")
sdse.number.isolates.t.cluster.refined <- nrow(sdse_df)

pyo_df <- read_csv("pyo_clusters_comm_3.csv")
# Remove singletons
pyo_singletons <- pyo_df %>% group_by(t.cluster.refined) %>% count() %>% 
  dplyr::filter(n==1)
pyo_df <- pyo_df %>% dplyr::filter(!t.cluster.refined %in% pyo_singletons)
pyo.number.isolates.t.cluster.refined <- nrow(pyo_df)

# Create vector of SDSE household IDs for transmission matrix
# Separate vectors for SDSE and pyo
sdse.hh.id <- unique(sdse_df$hh) # order of households in transmission matrix
sdse.number.hh <- length(sdse.hh.id) # number of households
sdse.number.hh.t.cluster.refined <- sdse.number.hh

# Create vector of pyo household IDs for transmission matrix
pyo.hh.id <- unique(pyo_df$hh) # order of households in transmission matrix
pyo.number.hh <- length(pyo.hh.id) # number of households
pyo.number.hh.t.cluster.refined <- pyo.number.hh
```
...
# Include only Community 3
sample_intervals_long <- sample_intervals_long %>% dplyr::filter(Community == 3)
...
sdse.mat.t.cluster.refined <- 
  matrix(0, nrow = sdse.number.hh, ncol = sdse.number.hh)
sdse.mat.t.cluster.refined.carriage <- 
  matrix(0, nrow = sdse.number.hh, ncol = sdse.number.hh)
rownames(sdse.mat.t.cluster.refined) <- sdse.hh.id
colnames(sdse.mat.t.cluster.refined) <- sdse.hh.id
rownames(sdse.mat.t.cluster.refined.carriage) <- sdse.hh.id
colnames(sdse.mat.t.cluster.refined.carriage) <- sdse.hh.id

pyo.t.cluster.refined <- pyo_df
pyo.transmission.determined <- 
  matrix(0, nrow = pyo.number.isolates.t.cluster.refined, ncol = 1)
pyo.mat.t.cluster.refined <- 
  matrix(0, nrow = pyo.number.hh, ncol = pyo.number.hh)
pyo.mat.t.cluster.refined.carriage <- 
  matrix(0, nrow = pyo.number.hh, ncol = pyo.number.hh)
rownames(pyo.mat.t.cluster.refined) <- pyo.hh.id
colnames(pyo.mat.t.cluster.refined) <- pyo.hh.id
rownames(pyo.mat.t.cluster.refined.carriage) <- pyo.hh.id
colnames(pyo.mat.t.cluster.refined.carriage) <- pyo.hh.id
...
# Transmission for SDSE
# Note sample interval starts at 24 for community 3 so need to subtract 23 for the lists
...
for (i in 1:sdse.number.isolates.t.cluster.refined) {
  ...
  if (length(possiblesource.i)>0) {
    ...
    sink.interval <- sample_intervals_long %>% 
      ...
      pull(Interval)
    if (sdse_time_determined[[sink.interval-23]][i,1]==0) {
      for (s in possiblesource.i) {
        source.date <- as.Date(sdse.t.cluster.refined$date[s], "%d/%m/%Y")
        source.interval <- sample_intervals_long %>% dplyr::filter(Date == source.date) %>%
          pull(Interval)
        # Error check
        ...
        hh.isolate.i <- which(grepl(hh.isolate.i, sdse.hh.id))
        # This is the household of the source isolate
        hh.isolate.j <- sdse.t.cluster.refined$hh[s]
        # This is the position in the matrix of this household
        hh.isolate.j <- which(grepl(hh.isolate.j, sdse.hh.id))
        # Add transmission event between these households based on sink interval:
        sdse_time_matrix[[sink.interval-23]][hh.isolate.j, hh.isolate.i] <-
          sdse_time_matrix[[sink.interval-23]][hh.isolate.j, hh.isolate.i] + 1
        ...
        if (sdse.t.cluster.refined$type[s]=="Aymptomatic_carriage") {
          sdse_time_matrix_carriage[[sink.interval-23]][hh.isolate.j, hh.isolate.i] <-
            sdse_time_matrix_carriage[[sink.interval-23]][hh.isolate.j, hh.isolate.i] + 1
        }
        # If source isolate j is a skin isolate, add to the skin matrix
        if (sdse.t.cluster.refined$type[s]=="Impetigo"){
          sdse_time_matrix_skin[[sink.interval-23]][hh.isolate.j, hh.isolate.i] <-
            sdse_time_matrix_skin[[sink.interval-23]][hh.isolate.j, hh.isolate.i] + 1
        }
      }
      # Tag isolate as transmission source being determined
      sdse_time_determined[[sink.interval-23]][i,1] <- 1
    }
  }
}
...
for (i in 1:pyo.number.isolates.t.cluster.refined) {
  ...
  if (length(possiblesource.i)>0) {
    ...
    if (pyo_time_determined[[sink.interval-23]][i,1]==0) {
      for (s in possiblesource.i) {
        ...
        # This is the position in the matrix of this household
        hh.isolate.i <- which(grepl(hh.isolate.i, pyo.hh.id))
        # This is the household of the source isolate
        hh.isolate.j <- pyo.t.cluster.refined$hh[s]
        # This is the position in the matrix of this household
        hh.isolate.j <- which(grepl(hh.isolate.j, pyo.hh.id))
        # Add transmission event between these households based on sink interval:
        pyo_time_matrix[[sink.interval-23]][hh.isolate.j, hh.isolate.i] <-
          pyo_time_matrix[[sink.interval-23]][hh.isolate.j, hh.isolate.i] + 1
        ...
        if (pyo.t.cluster.refined$type[s]=="Aymptomatic_carriage"){
          pyo_time_matrix_carriage[[sink.interval-23]][hh.isolate.j, hh.isolate.i] <-
            pyo_time_matrix_carriage[[sink.interval-23]][hh.isolate.j, hh.isolate.i] + 1
        } else if (pyo.t.cluster.refined$type[s]=="Impetigo") {
          pyo_time_matrix_skin[[sink.interval-23]][hh.isolate.j, hh.isolate.i] <-
            pyo_time_matrix_skin[[sink.interval-23]][hh.isolate.j, hh.isolate.i] + 1
        }
      }
      # Tag isolate as transmission source being determined
      pyo_time_determined[[sink.interval-23]][i,1] <- 1
    }
  }
}
```
...
hh.size <- hh.size %>% dplyr::filter(community == 3)
number.hh <- length(hh.size$hh)

# Read in proportion of positive SDSE and pyo positives in each household
hh_prop <- read_csv("HH_prop_comm_3.csv")

# First take each time point and convert to data frame
union_all_df <- tibble(from = character(), to = character(), weight_1 = numeric(),
                       weight_2 = numeric(), color = character(),
                       weight = numeric(), interval = numeric())
union_throat_df <- tibble(from = character(), to = character(), weight_1 = numeric(),
                          weight_2 = numeric(), color = character(),
                          weight = numeric(), interval = numeric())
...
```{r}
# Plot overlapping networks over time
...
V(timed_graph)$size <- (hh.size.temp$size)^0.5/50
# Set proportions
...
E(timed_graph)$interval2 <- dplyr::recode(E(timed_graph)$interval, 
                                   "3" = "Aug 2004", "6" = "Nov 2004", 
                                   "7" = "Dec 2004", "8" = "Jan 2005", 
                                   "9" = "Feb 2005", "11" = "May 2005", 
                                   "12" = "Jun 2005")
...
transmission_net <- ggraph(timed_graph, layout = "manual", 
  ...
  facet_wrap(~ factor(interval2, levels = c("Aug 2004", "Nov 2004", "Dec 2004",
                                            "Jan 2005", "Feb 2005", "May 2005", 
                                            "Jun 2005"))) + 
...
permute_plot <- permute_overlap %>% ggplot(aes(x = overlap_edges)) + 
  ...
  geom_vline(xintercept = nrow(union_all_df %>% filter(color == "#20854E")), 
             linetype = "dashed", colour = "firebrick", size = 1) +
